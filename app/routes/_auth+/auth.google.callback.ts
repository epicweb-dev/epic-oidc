...
import {
	...
	prefilledProfileKey,
} from './onboarding_.google.tsx'

export const ROUTE_PATH = '/auth/google/callback'
...
export async function loader({ request }: DataFunctionArgs) {
	...
	const authResult = await authenticator
		.authenticate(GOOGLE_PROVIDER_NAME, request, { throwOnError: true })
		.then(
	...
	// If we're already logged in, then link the Google account
	if (userId) {
	...
	// if the google email matches a user in the db, then link the account and
	// make a new session
	...
	verifySession.set(googleIdKey, profile.id)
